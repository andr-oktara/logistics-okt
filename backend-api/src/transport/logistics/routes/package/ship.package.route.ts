...
export const ShipPackageRoute = (logger: LoggerHandler): express.Router => {
  const router = express.Router();
  const packageService = new PackageServiceClient().init({ logger });

  router.put("/package/:id/ship", param("id").isUUID("4"), async (req, res) => {
    ...
    try {
      await packageService.ship(receivedData.id);
